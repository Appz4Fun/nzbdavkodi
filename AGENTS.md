...
- Credentialed test recipes (`just extreme-tests` and the functional/live-service tests) read `.env` from the current directory by default. The canonical `.env` lives in the primary checkout's repository root (the directory containing `.claude/worktrees/`). When running from a git worktree, do not create a worktree-local `.env`; set `EXTREME_ENV_FILE=<primary-checkout>/.env` instead.
