...
_FALLBACK_RES_RE = re.compile(r"(?i)\b(2160p|1080p|1080i|720p|480p|4K)\b")
_FALLBACK_CODEC_RE = re.compile(r"(?i)\b(x265|h\.?265|hevc|x264|h\.?264|avc|av1|vp9)\b")
_FALLBACK_ATMOS_RE = re.compile(r"(?i)\batmos\b")
_FALLBACK_TRUEHD_RE = re.compile(r"(?i)\btruehd\b")
_FALLBACK_DTSHD_RE = re.compile(r"(?i)\bdts[-. ]?hd[-. ]?ma\b")
_FALLBACK_DDP_RE = re.compile(r"(?i)\bddp?5[. ]1|eac3|dd\+|dolby\.digital\.plus\b")
_FALLBACK_DD_RE = re.compile(r"(?i)\bac3|dd[. ]?5[. ]1|dolby\.digital\b")
_FALLBACK_AAC_RE = re.compile(r"(?i)\baac\b")
_FALLBACK_DTS_RE = re.compile(r"(?i)\bdts\b")
_FALLBACK_DV_RE = re.compile(r"(?i)\b(dv|dovi|dolby[. ]?vision)\b")
...
_FALLBACK_CHANNELS_RE = re.compile(r"\b(7\.1|5\.1|2\.0)\b")
...
_FALLBACK_YEAR_RE = re.compile(r"[. (](\d{4})[. )]")
_FALLBACK_UPSCALED_RE = re.compile(r"(?i)\bupscale[d]?\b")
# Group (last segment after hyphen). Scene groups can contain hyphens and
# underscores, e.g. GROUP-NAME or GROUP_NAME.
_FALLBACK_GROUP_RE = re.compile(r"-([A-Za-z0-9][A-Za-z0-9_-]*)(?:\.[a-z]{2,4})?$")
...
def _fallback_parse(title):
    ...
    # Resolution
    m = _FALLBACK_RES_RE.search(t)
    if m:
        result["resolution"] = m.group(1)

    # Codec
    m = _FALLBACK_CODEC_RE.search(t)
    if m:
        result["codec"] = m.group(1).lower()

    # Audio
    audio = []
    if _FALLBACK_ATMOS_RE.search(t):
        audio.append("Atmos")
    if _FALLBACK_TRUEHD_RE.search(t):
        audio.append("TrueHD")
    if _FALLBACK_DTSHD_RE.search(t):
        audio.append("DTS-HD MA")
    if _FALLBACK_DDP_RE.search(t):
        audio.append("DD+")
    if _FALLBACK_DD_RE.search(t):
        audio.append("DD")
    if _FALLBACK_AAC_RE.search(t):
        audio.append("AAC")
    if _FALLBACK_DTS_RE.search(t) and not audio:
        audio.append("DTS")
    result["audio"] = audio

    # HDR
    hdr = []
    if _FALLBACK_DV_RE.search(t):
        hdr.append("DV")
    ...
    if _FALLBACK_HDR10PLUS_RE.search(t):
        hdr.append("HDR10+")
    # `hdr10` without the optional `0` would match `hdr1`; require the digit.
    elif _FALLBACK_HDR10_RE.search(t):
        hdr.append("HDR10")
    if _FALLBACK_HLG_RE.search(t):
        hdr.append("HLG")
    result["hdr"] = hdr

    # Quality / Source
    m = _FALLBACK_QUALITY_RE.search(t)
    if m:
    ...
    # Edition
    m = _FALLBACK_EDITION_RE.search(t)
    if m:
        result["edition"] = m.group(1).replace(".", " ")

    # Channels
    m = _FALLBACK_CHANNELS_RE.search(t)
    if m:
        result["channels"] = m.group(1)

    # Year
    m = _FALLBACK_YEAR_RE.search(t)
    if m:
        yr = int(m.group(1))
        if 1920 <= yr <= 2100:
            result["year"] = yr
    ...
    # Upscaled
    if _FALLBACK_UPSCALED_RE.search(t):
        result["upscaled"] = True

    # Group
    m = _FALLBACK_GROUP_RE.search(title)
    if m:
