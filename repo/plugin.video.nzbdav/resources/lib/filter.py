...
# Pre-compiled regular expressions for fallback parsing
_FB_RES_RE = re.compile(r"\b(2160p|1080p|1080i|720p|480p|4K)\b", re.IGNORECASE)
_FB_CODEC_RE = re.compile(
    r"\b(x265|h\.?265|hevc|x264|h\.?264|avc|av1|vp9)\b", re.IGNORECASE
)
_FB_ATMOS_RE = re.compile(r"\batmos\b", re.IGNORECASE)
_FB_TRUEHD_RE = re.compile(r"\btruehd\b", re.IGNORECASE)
_FB_DTSHD_RE = re.compile(r"\bdts[-. ]?hd[-. ]?ma\b", re.IGNORECASE)
_FB_DDPLUS_RE = re.compile(
    r"\bddp?5[. ]1|eac3|dd\+|dolby.digital.plus\b", re.IGNORECASE
)
_FB_DD_RE = re.compile(r"\bac3|dd[. ]?5[. ]1|dolby.digital\b", re.IGNORECASE)
_FB_AAC_RE = re.compile(r"\baac\b", re.IGNORECASE)
_FB_DTS_RE = re.compile(r"\bdts\b", re.IGNORECASE)

_FB_DV_RE = re.compile(r"\b(dv|dovi|dolby[. ]?vision)\b", re.IGNORECASE)
_FB_HDR10PLUS_RE = re.compile(r"\b(hdr10\+|hdr10plus)\b", re.IGNORECASE)
_FB_HDR10_RE = re.compile(r"\bhdr10\b", re.IGNORECASE)
_FB_HLG_RE = re.compile(r"\bhlg\b", re.IGNORECASE)

_FB_QUALITY_RE = re.compile(
    r"\b(remux|blu[-. ]?ray|bdrip|web[-. ]?dl|webrip|hdtv|dvdrip|hdrip)\b",
    re.IGNORECASE,
)
_FB_EDITION_RE = re.compile(
    r"\b(uncut|unrated|director'?s[. ]?cut|extended[. ]?cut"
    r"|recut|theatrical|imax|special[. ]?edition)\b",
    re.IGNORECASE,
)
_FB_CHANNELS_RE = re.compile(r"\b(7\.1|5\.1|2\.0)\b")
_FB_YEAR_RE = re.compile(r"[. (](\d{4})[. )]")
_FB_UPSCALED_RE = re.compile(r"\bupscale[d]?\b", re.IGNORECASE)
_FB_GROUP_RE = re.compile(r"-([A-Za-z0-9][A-Za-z0-9_-]*)(?:\.[a-z]{2,4})?$")
...
def _fallback_parse(title):
    ...
    # Resolution
    m = _FB_RES_RE.search(t)
    if m:
        result["resolution"] = m.group(1)

    # Codec
    m = _FB_CODEC_RE.search(t)
    if m:
        result["codec"] = m.group(1).lower()

    # Audio
    audio = []
    if _FB_ATMOS_RE.search(t):
        audio.append("Atmos")
    if _FB_TRUEHD_RE.search(t):
        audio.append("TrueHD")
    if _FB_DTSHD_RE.search(t):
        audio.append("DTS-HD MA")
    if _FB_DDPLUS_RE.search(t):
        audio.append("DD+")
    if _FB_DD_RE.search(t):
        audio.append("DD")
    if _FB_AAC_RE.search(t):
        audio.append("AAC")
    if _FB_DTS_RE.search(t) and not audio:
        audio.append("DTS")
    result["audio"] = audio

    # HDR
    hdr = []
    if _FB_DV_RE.search(t):
        hdr.append("DV")
    ...
    if _FB_HDR10PLUS_RE.search(t):
        hdr.append("HDR10+")
    # `hdr10` without the optional `0` would match `hdr1`; require the digit.
    elif _FB_HDR10_RE.search(t):
        hdr.append("HDR10")
    if _FB_HLG_RE.search(t):
        hdr.append("HLG")
    result["hdr"] = hdr

    # Quality / Source
    m = _FB_QUALITY_RE.search(t)
    if m:
    ...
    # Edition
    m = _FB_EDITION_RE.search(t)
    if m:
        result["edition"] = m.group(1).replace(".", " ")

    # Channels
    m = _FB_CHANNELS_RE.search(t)
    if m:
