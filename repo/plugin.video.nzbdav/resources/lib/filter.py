...
_RE_RESOLUTION = re.compile(r"(?i)\b(2160p|1080p|1080i|720p|480p|4K)\b")
_RE_CODEC = re.compile(r"(?i)\b(x265|h\.?265|hevc|x264|h\.?264|avc|av1|vp9)\b")
_RE_AUDIO_ATMOS = re.compile(r"(?i)\batmos\b")
_RE_AUDIO_TRUEHD = re.compile(r"(?i)\btruehd\b")
_RE_AUDIO_DTSHD = re.compile(r"(?i)\bdts[-. ]?hd[-. ]?ma\b")
_RE_AUDIO_DDP = re.compile(r"(?i)\bddp?5[. ]1|eac3|dd\+|dolby.digital.plus\b")
_RE_AUDIO_DD = re.compile(r"(?i)\bac3|dd[. ]?5[. ]1|dolby.digital\b")
_RE_AUDIO_AAC = re.compile(r"(?i)\baac\b")
_RE_AUDIO_DTS = re.compile(r"(?i)\bdts\b")
_RE_HDR_DV = re.compile(r"(?i)\b(dv|dovi|dolby[. ]?vision)\b")
_RE_HDR_HDR10PLUS = re.compile(r"(?i)\b(hdr10\+|hdr10plus)\b")
_RE_HDR_HDR10 = re.compile(r"(?i)\bhdr10\b")
_RE_HDR_HLG = re.compile(r"(?i)\bhlg\b")
...
def _fallback_parse(title):
    ...
    # Audio
    audio = []
    if _RE_AUDIO_ATMOS.search(t):
        audio.append("Atmos")
    if _RE_AUDIO_TRUEHD.search(t):
        audio.append("TrueHD")
    if _RE_AUDIO_DTSHD.search(t):
        audio.append("DTS-HD MA")
    if _RE_AUDIO_DDP.search(t):
        audio.append("DD+")
    if _RE_AUDIO_DD.search(t):
        audio.append("DD")
    if _RE_AUDIO_AAC.search(t):
        audio.append("AAC")
    if _RE_AUDIO_DTS.search(t) and not audio:
        audio.append("DTS")
    result["audio"] = audio

    # HDR
    hdr = []
    if _RE_HDR_DV.search(t):
        hdr.append("DV")
    ...
    if _RE_HDR_HDR10PLUS.search(t):
        hdr.append("HDR10+")
    # `hdr10` without the optional `0` would match `hdr1`; require the digit.
    elif _RE_HDR_HDR10.search(t):
        hdr.append("HDR10")
    if _RE_HDR_HLG.search(t):
        hdr.append("HLG")
    result["hdr"] = hdr
