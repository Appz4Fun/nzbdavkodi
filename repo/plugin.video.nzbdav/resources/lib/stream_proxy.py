...
_HLS_SEGMENT_RE = re.compile(r"seg_0*(\d+)\.(m4s|ts)")
_CONTENT_RANGE_RE = re.compile(r"^bytes\s+0-0/(\d+)$")
...
class HlsProducer:
    ...
    def generated_playlist_body(self):
        ...
        text = _HLS_SEGMENT_RE.sub(_normalize_segment, text)
        ...
        if content_length_hint > 0:
            try:
                ...
                with urlopen(  # nosec B310 — URL from user-configured nzbdav/WebDAV setting
                    ...
                    if status is None:
                        status = resp.getcode()
                    match = _CONTENT_RANGE_RE.match(cr.strip())
