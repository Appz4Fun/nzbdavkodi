...
def test_normalize_api_endpoint_adds_default_api_path():
    assert normalize_api_endpoint("https://api.nzbgeek.info") == (
        "https://api.nzbgeek.info/api"
    )
    assert normalize_api_endpoint("http://localhost:5076") == (
...
def test_normalize_api_endpoint_preserves_existing_paths_query_and_fragment():
    assert normalize_api_endpoint("https://api.nzbgeek.info/") == (
        "https://api.nzbgeek.info/api"
    )
    assert normalize_api_endpoint("https://api.nzbgeek.info/api/") == (
        "https://api.nzbgeek.info/api"
    )
    assert normalize_api_endpoint("https://tabula-rasa.pw/api/v1/") == (
        "https://tabula-rasa.pw/api/v1"
    )
    assert normalize_api_endpoint("https://idx.example/newznab?foo=1#caps") == (
        "https://idx.example/newznab?foo=1#caps"
    )
