...
|---|---|
| Playback never starts | `kodi.log` around the `prepare_stream` call — look for `Proxy ready (remux=…, faststart=…)`. Which tier was chosen? Is the URL returned correct? Was `setResolvedUrl` called? A `background service unreachable` error means `/prepare` failed. |
