...
| **Matroska remux** (`ffmpeg -c copy -f matroska pipe:1`) | Yes | No HTTP range; a seek more than 10 MB away respawns ffmpeg with `-ss`, so seeking is **approximate** to the nearest keyframe. |
